"""
Gemini 버전 llm.py (Google AI Studio API용)

[사용 환경]
- 회사 IT에서 Gemini API 키만 발급받은 경우
- 환경변수: GEMINI_API_KEY
...
[사용법]
기존 app.py / server.py 에서 `from llm import get_recommendation` 으로 import 하던 부분을
`from llm_gemini import get_recommendation` 으로 바꾸기만 하면 됨.
또는 이 파일을 llm.py 로 이름만 바꿔서 덮어써도 됨.
"""
